...
class UltraGCN(nn.Module):
    def __init__(self, beta_UD, beta_iD, ii_constraint_mat, ii_neighbor_mat):
        ...
        device = 'cuda'
        self.user_embeds = nn.Embedding(5012, self.embedding_dim).to(device)
    ...
    def forward(self, user, pos_items, neg_items):
        ...
        loss = self.cal_loss_L(user, pos_items, neg_items, omega_weight)
        loss += self.gamma * self.norm_loss()

        loss += self.lambda_ * self.cal_loss_I(user, pos_items)
        return loss
...
def train(model, optimizer, train_loader,interacted_items,train_text_dict, test_loader, mask, test_ground_truth_list, index_to_champion_name,mode):
    ...
    for batch, x  in enumerate(train_loader):
        ...
        with autocast(device_type = 'cuda'):
            loss = model(users.cuda(), pos_items, neg_items.cuda())
    ...
    F1_score, Precision, Recall, NDCG = test(model, test_loader, test_ground_truth_list, train_text_dict, mask, 5,600) #이 부분 갈아끼워야함.
    ...
    with torch.no_grad():
        ...
        for idx, batch_users in enumerate(test_loader):
            with autocast(device_type = 'cuda'):
                ...
                rating = model.test_foward(batch_users[0].cuda()).cpu()
...
if __name__ == "__main__":
    ...
    train_loader_pick = data.DataLoader(train_data_pick, batch_size=1024, shuffle=True, num_workers=22)
    ...
    #ultragcn = UltraGCN(beta_UD_win, beta_iD_win, ii_constraint_mat_win.cuda(),ii_neighbor_mat_win.cuda()).cuda()

    ultragcn = UltraGCN(beta_UD_pick, beta_iD_pick, ii_constraint_mat_pick.cuda(), ii_neighbor_mat_pick.cuda()).cuda()
    ...
    # wandb 초기화
    wandb.init(project='Ultragcn-training_test5')
    wandb.run.name = "train with part data pickrate55555"
    wandb.run.save()
    ...
    for iter in range(75):
       ...
       # ultragcn,F1_score_win, Precision_win, Recall_win, NDCG_win  =\
        ...
        if NDCG_pick > best_ndcg_pick:
            ...
            torch.save(best_model_pick, os.path.join(directory, 'part_best_gcn_pick.pth'))
